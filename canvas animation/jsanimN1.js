...
/* Tegner og står for bevægelsen, og hit-detection af cirklerne */
...
class Umo {
    ...
    /* udregner arialet til hit-detection */
    calcArea() {
...
/* fjerner en cirkel fra arrayet når den bliver ramt */
const redraw = function () {
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      /* Den sidste levende cirkle bliver sat ind i midten af canvasset */
      if (arr.length <= 1) {
        arr[0].dx = 0;
        arr[0].dy = 0;
...
const repeater = function () {
    setInterval(redraw, 11);
}


 /* Tegner 20 cirkler til canvas't som får forskellige farver ved hjælp af den tidliger funktion */
const init = function () {
...
function hitCalc(obj){
  ...
  let ss = Math.sqrt((obj.x - this.x)^2 + (obj.y - this.y)^2)
  //console.log(ss)
  return ss;
}
